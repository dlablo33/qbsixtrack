    ...
    <div class="row">
        <div class="col-12">
            <h1>Remisión</h1>
                ...
                <p><strong>Fecha:</strong> {{ \Carbon\Carbon::parse($factura->fecha_create)->format('d/m/Y') }}</p>
                <p><strong>Bol:</strong> {{ $factura->bol}} </p>
            ...
            <div class="client-info">
                ...
                <p><strong>Cliente:</strong> {{ $factura->cliente_name }}</p>
                <p><strong>RFC:</strong> {{ $factura->customer ? $factura->customer->RFC : 'N/A' }}</p>
            </div>
            <hr>
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>{{ $factura->producto_id}}-{{$factura->producto_name}}</td>
                            <td>{{ $factura->producto_name }}</td>
                            <td>${{  number_format(number_format($factura->total / $factura->cantidad, 2, '.', ''), 0, ',', ',') }}</td>
            ...
            <div class="total-info">
                <p><strong>Total con Letra:</strong> {{ convertirNumeroALetras($factura->total) }}</p>
                <p class="left-align"><strong>Total:</strong> ${{  number_format(number_format($factura->total, 2, '.', ''), 0, ',', ',') }}</p>
            </div>
            <hr>
            <div class="note">
